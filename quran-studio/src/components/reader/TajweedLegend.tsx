...
/** The Tafkhim group has no rule in the quran.com dataset, so its swatch is
 *  drawn from the token directly rather than from a `data-tajweed` span. */
const TAFKHIM_SWATCH = "var(--tj-tafkhim, #00369E)";
...
{TAJWEED_GROUPS.map((group) => {
            ...
            // Tafkhim carries no rule in the dataset — a swatch, not a list.
            if (group === "tafkhim") {
              return (
                <section key={group}>
                  ...
                  <ul className="space-y-1.5">
                    <li className="flex items-baseline gap-2.5">
                      <span
                        ...
                        style={{ backgroundColor: TAFKHIM_SWATCH }}
                        aria-hidden
                      />
                      <span className="min-w-0">
                        <span className="text-[0.8125rem] text-fg">{t("tajweed.tafkhim.label")}</span>
                        <span className="ml-1.5 text-[0.75rem] text-fg-subtle">
                          {t("tajweed.tafkhim.hint")}
                        </span>
                      </span>
                    </li>
                  </ul>
                ...
                <ul className="space-y-1.5">
                  {rules.map((rule) => (
                    <li key={rule} className="flex items-baseline gap-2.5">
                      ...
                      />
                      <span className="min-w-0">
                        <span className="text-[0.8125rem] text-fg">{t(`tajweed.${rule}.label`)}</span>
                        <span className="ml-1.5 text-[0.75rem] text-fg-subtle">
                          {t(`tajweed.${rule}.hint`)}
                        </span>
                      </span>
